...
const { generateStudentNotifications, createIfNotExists } = require("../services/notificationGeneratorService");
...
// @desc    Create a medication reminder notification (for Senior Citizen)
// @route   POST /api/notifications/medication-reminder
// @access  Private
exports.createMedicationNotification = async (req, res) => {
  try {
    const userId = req.user._id;
    const { medicationId, medicineName, dosage, time } = req.body;
    ...
    const sourceId = `${medicationId || "med"}_${todayStr}`;

    const notification = await createIfNotExists({
      userId,
      category: "Medication",
      title: "💊 Medication Reminder",
      message: `It's time to take your ${medicineName}${dosage ? ` (${dosage})` : ""}.`,
      type: "medication_reminder",
      icon: "FiClock",
      priority: "High",
      link: "/senior/medications",
      sourceType: "SeniorMedication",
      sourceId,
    });
    ...
    return res.status(201).json({
      success: true,
      message: "Medication notification created.",
      unreadCount,
      data: notification,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to create medication notification.",
    });
  }
};
