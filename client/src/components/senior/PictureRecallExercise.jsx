...
const TARGET_PICTURES = ["🍎", "☕", "🌸", "📚", "🕶️"];
const ALL_CHOICES = ["☕", "🍎", "🚗", "📚", "🌸", "🐱", "🕶️", "🏠", "⚽"];

function PictureRecallExercise({ onClose, onExerciseComplete }) {
  ...
  const toggleSelectPicture = (pic) => {
    if (phase !== "recall") return;
    if (selectedPictures.includes(pic)) {
      setSelectedPictures(selectedPictures.filter((p) => p !== pic));
    } else {
  ...
  const handleSubmitRecall = async () => {
    ...
    selectedPictures.forEach((pic) => {
      if (TARGET_PICTURES.includes(pic)) {
        correctCount++;
      } else {
        incorrectCount++;
      }
    ...
    const accuracy = Math.round((correctCount / TARGET_PICTURES.length) * 100);
    ...
    const resultObj = {
      score,
      accuracy: `${correctCount}/${TARGET_PICTURES.length}`,
    ...
    if (token) {
      try {
        ...
        console.error("Error saving Picture Recall result:", err);
      }
    }
  };
    ...
    <div className="ns-card p-4 position-relative">
      ...
      <div className="d-flex align-items-center justify-content-between mb-4 pb-3 border-bottom border-secondary border-opacity-25">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h4 className="text-white fw-bold mb-0">🖼️ Picture Recall</h4>
            <span className="text-muted small">Remember objects & identify them later</span>
          </div>
        </div>
      ...
      {phase === "instructions" && (
        <div className="text-center py-4 px-3 max-w-lg mx-auto">
          ...
          <p className="text-white-50 small mb-4" style={{ lineHeight: "1.6" }}>
            You will see a set of <strong>5 recognizable object pictures</strong> for <strong>5 seconds</strong>.
            Remember as many as you can, then select them from a larger list.
          </p>
          <button
            type="button"
      ...
      {phase === "memorize" && (
        <div className="text-center py-4 px-3 rounded-4" style={{ background: "rgba(30, 41, 59, 0.8)", border: "1px solid rgba(255, 255, 255, 0.08)" }}>
          <span className="badge rounded-pill bg-warning text-dark px-3 py-1 fw-bold mb-3">
            ⏱️ Remember objects in {countdown}s
          </span>

          <h5 className="text-white fw-semibold mb-4">Remember these objects:</h5>

          <div className="d-flex flex-wrap align-items-center justify-content-center gap-3 my-4">
            {TARGET_PICTURES.map((pic, idx) => (
              <span
                key={idx}
          ...
          <p className="text-muted small mb-0">The pictures will hide automatically when countdown finishes.</p>
        </div>
      )}
      ...
      {phase === "recall" && (
        <div className="text-center py-4 px-3 rounded-4" style={{ background: "rgba(30, 41, 59, 0.8)", border: "1px solid rgba(255, 255, 255, 0.08)" }}>
          <h5 className="text-white fw-semibold mb-3">Which objects did you see?</h5>
          <span className="text-muted small d-block mb-4">Click to select all pictures you remember (Selected: {selectedPictures.length}):</span>

          {/* Options Grid */}
          <div className="row g-3 max-w-md mx-auto mb-4">
            {ALL_CHOICES.map((pic, idx) => {
              const isSelected = selectedPictures.includes(pic);
              return (
                <div key={idx} className="col-4">
                  <button
                    type="button"
                    className="btn w-100 py-3 rounded-4 fs-1 text-white d-flex align-items-center justify-content-center transition-all"
                    style={{
                    ...
                    onClick={() => toggleSelectPicture(pic)}
                  >
                    <span>{pic}</span>
                  </button>
                </div>
...
export default PictureRecallExercise;
